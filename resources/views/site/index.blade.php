...
<div class="inputbox colorbox">
  ...
  <div class="flex items-center">
    <a class="flex" href="{{ route('sites.create', ['order' => request()->order, 'favorites' => request()->favorites, 'tag' => request()->tag, 'color'=> request()->color]) }}">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  ...
  <div class="flex items-center">
    <a class="flex" href="{{ route('sites.index', ['order' => request()->order, 'favorites' => 1, 'tag' => request()->tag, 'color'=> request()->color]) }}">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    @if(request()->favorites)
      <button class="ml-2 form-input" onclick="location.href='{{ route('sites.index', ['order' => request()->order, 'tag' => request()->tag, 'color'=> request()->color]) }}'">この条件を削除</button>
    @endif
  </div>
  <div class="flex items-center">
    <a class="flex" href="{{ route('sites.tags', ['order' => request()->order, 'favorites' => request()->favorites, 'color' => request()->color]) }}">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    ：{{ request()->tag ?? "無し" }}
    @if(request()->tag)
    <button class="ml-2 form-input" onclick="location.href='{{ route('sites.index', ['order' => request()->order, 'favorites' => request()->favorites, 'color'=> request()->color]) }}'">この条件を削除</button>
    @endif
  </div>
  <div>
    <form class="flex items-center" action="{{ route('sites.index', ['favorites' => request()->favorites, 'tag' => request()->tag, 'color'=> request()->color]) }}" method="get">
      ...
      ：{{ request()->color ?? "指定無し" }}
      @if(request()->color)
        <button class="ml-2 form-input" onclick="location.href='{{ route('sites.index', ['order' => request()->order, 'favorites' => request()->favorites, 'tag' => request()->tag]) }}'">この条件を削除</button>
      @endif
    </form>
  </div>
  <div>
    <form class="flex items-center" action="{{ route('sites.index', ['order' => request()->order, 'favorites' => request()->favorites, 'tag' => request()->tag, 'color'=> request()->color]) }}" method="get">
      <input type="hidden" name="tag" value="{{ request()->tag }}">
      <input type="hidden" name="favorites" value="{{ request()->favorites }}">
